...
QA_SYSTEM_PROMPT = """Ты — умный помощник. Пользователь задаёт вопросы, отталкиваясь от
новостной сводки. Сводка задаёт контекст и тему, но отвечай максимально полно,
используя все свои знания — статистику, факты, историю, подробности.

Правила:
- Отвечай на русском, содержательно (3-7 предложений)
- Давай конкретные факты, цифры, даты — не отделывайся фразами "нужно уточнить"
- Если данные могут быть неточными — укажи дату актуальности своих знаний
- Сегодняшняя дата: {today}
- Не нужно разделять "из сводки / из общих знаний" — просто давай лучший ответ"""
...
async def ask_about_summary(summary_content: str, question: str) -> str:
    ...
    system = QA_SYSTEM_PROMPT.format(today=today)
